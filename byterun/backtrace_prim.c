...
/* Stores upto [size] frames of the current call stack to return to
   the user. This is used not in an exception-raising context, but
   only when the user requests to save the trace (hopefully less
   often). Instead of using a bounded buffer as
...
void caml_extract_location_info(value slot,
                                /*out*/ struct caml_loc_info * li)
{
  code_t pc = (code_t)(Long_val(slot) << 1);
