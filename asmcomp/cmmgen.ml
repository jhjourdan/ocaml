...
let make_alloc_generic set_fn tag wordsize args dbg =
  if wordsize <= Config.max_young_wosize then
    Cop(Calloc dbg, Cconst_blockheader(block_header tag wordsize) :: args)
  else begin
    ...
    Clet(id,
         Cop(Cextcall("caml_alloc", typ_val, true, dbg),
...
let rec expr_size env = function
  ...
  | Uprim (Pduprecord ((Record_regular | Record_inlined _), sz, _), _, _) ->
      RHS_block sz
  | Uprim (Pduprecord (Record_extension, sz, _), _, _) ->
      RHS_block (sz + 1)
...
let box_number dbg bn arg =
  match bn with
  | Boxed_float -> box_float dbg arg
  | Boxed_integer bi -> box_int dbg bi arg
...
let rec transl env e =
  match e with
    Uvar id ->
      ...
      | Some (unboxed_id, bn) ->
         (* TODO : debug info *)
         box_number Debuginfo.none bn (Cvar unboxed_id)
      end
  | Uconst sc ->
      transl_constant sc
  | Uclosure(fundecls, []) ->
  ...
  | Uprim(prim, args, dbg) ->
      ...
      | (Pmakearray (kind, _, loc), args) ->
          transl_make_array env kind args
                            (Debuginfo.from_location Debuginfo.Dinfo_call loc)
      | (Pduparray (kind, _), [Uprim (Pmakearray (kind', _, loc), args, _dbg)]) ->
...
and transl_make_array env kind args dbg =
  match kind with
  | Pgenarray ->
      Cop(Cextcall("caml_make_array", typ_val, true, dbg),
          [make_alloc 0 (List.map (transl env) args) dbg])
...
and transl_ccall env prim args dbg =
  ...
  let typ_res, wrap_result =
    ...
    | Unboxed_integer Pint64 when size_int = 4 -> ([|Int; Int|], box_int dbg Pint64)
...
and transl_prim_1 env p arg dbg =
  ...
  | Poffsetint n ->
      if no_overflow_lsl n 1 then
        ...
        transl_prim_2 env Paddint arg (Uconst (Uconst_int n)) Debuginfo.none
  | Poffsetref n ->
      return_unit
