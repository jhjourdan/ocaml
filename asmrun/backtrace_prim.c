...
void caml_extract_location_info(value slot, /*out*/ struct caml_loc_info * li)
{
  uintnat infoptr;
  ...
  if(Is_long(slot)) {
    d = (frame_descr*)(Long_val(slot) << 1);
    alloc_id = 0;
  } else {
    d = (frame_descr*)(Long_val(Field(slot, 0)) << 1);
