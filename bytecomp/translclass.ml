...
let output_methods tbl methods lam =
  ...
  | _ ->
      lsequence (mkappl(oo_prim "set_methods",
                        [Lvar tbl; Lprim(Pmakeblock(0,Immutable, Location.none), methods)]))
        lam
...
let transl_class ids cl_id pub_meths cl vflag =
  ...
  let make_envs lam =
    Llet(StrictOpt, envs,
         ...
         Lprim(Pmakeblock(0, Immutable, Location.none), lenv :: linh_envs)),
         lam)
  and def_ids cla lam =
    Llet(StrictOpt, env2,
  ...
  and lcache lam =
    ...
    Llet(Strict, cached,
         mkappl (oo_prim "lookup_tables",
                [Lvar tables; Lprim(Pmakeblock(0, Immutable, Location.none), inh_keys)]),
         lam)
