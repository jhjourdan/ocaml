  ...
  match ext.ext_kind with
    Text_decl(args, ret) ->
      Lprim(prim_set_oo_id,
            [Lprim(Pmakeblock(Obj.object_tag, Mutable, Location.none),
...
let transl_package component_names target_name coercion =
  let components =
    Lprim(Pmakeblock(0, Immutable, Location.none), List.map get_component component_names) in
...
let transl_store_package component_names target_name coercion =
  ...
  | Tcoerce_structure (pos_cc_list, id_pos_list) ->
      let components =
        Lprim(Pmakeblock(0, Immutable, Location.none), List.map get_component component_names)
      in
