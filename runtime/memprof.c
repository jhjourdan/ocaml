...
/* Executes the callback assoctiated with the given tracked block, and
   ...
   Returns [Val_long(1)] if [t] is still in the list after the callback,
   and [Val_long(0)] if it has been dropped and freed.
...
static value do_callback_exn(struct tracked* t)
{
  CAMLparam0();
...
{
  if(caml_memprof_suspended) return Val_unit;
...
void caml_memprof_track_young(tag_t tag, uintnat wosize, int from_caml)
{
  ...
  t->cb_kind = CB_ALLOC;
  still_tracked = Long_val(caml_raise_if_exception(do_callback_exn(t)));
